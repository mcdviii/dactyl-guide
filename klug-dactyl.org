...
#+ATTR_REVEAL: :frag (fade-in fade-in fade-in)
- Advantages:
- Disadvantages: Expensive (PCB is $)
- Open Source: Yes?
#+BEGIN_NOTES
...
option for the same version. The LightCycle version of the Dactyl has fewer thumb cluster switch positions, and was designed to match with the [[http://matias.ca/switches/][Matias
